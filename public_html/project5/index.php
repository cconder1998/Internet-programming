<?php
    ...
    $UNF_EMAIL_ID = 123456789; // Email ID (see Grades section in Canvas)
    ...
    function checkout($name, $address) 
	{
			...
			<body>
				...
				if(count($movies) == 0):
					...
					foreach($movies as $movieID): 
						$result = file_get_contents('http://www.omdbapi.com/?apikey='.$GLOBALS['OMDB_API_KEY'].'&i=' . $movieID .'&type=movie&r=json');
		...
		echo $message;
        $result = sendMail($GLOBALS['UNF_EMAIL_ID'], $address, $name, 'myMovies Express! Receipt', $message);
		$resultMessage = "";
		switch($result) 
        {
            case 0:
